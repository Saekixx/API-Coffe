...
public class GlobalExceptionHandler {
        ...
        public ResponseEntity<ResponseGlobal<Void>> handleResourceNotFound(ResourceNotFoundException ex,
        ...
        public ResponseEntity<ResponseGlobal<Void>> handleBusinessRule(BusinessException ex,
                        HttpServletRequest request) {
        ...
        public ResponseEntity<ResponseGlobal<Void>> handleValidationException(MethodArgumentNotValidException ex,
                        HttpServletRequest request) {
        ...
        public ResponseEntity<ResponseGlobal<Void>> handleBadCredentials(BadCredentialsException ex,
                        HttpServletRequest request) {
                log.warn("Credenciales inválidas en [{}]: {}", request.getRequestURI(), ex.getMessage());
                return ResponseEntity
                                .status(HttpStatus.UNAUTHORIZED)
                                .body(ResponseGlobal.error(HttpStatus.UNAUTHORIZED.value(),
                                                "Credenciales incorrectas"));
        }

        // 403 - Acceso denegado lanzado por Spring Security (@PreAuthorize)
        @ExceptionHandler(org.springframework.security.access.AccessDeniedException.class)
        public ResponseEntity<ResponseGlobal<Void>> handleSpringAccessDenied(
                        org.springframework.security.access.AccessDeniedException ex,
                        HttpServletRequest request) {
                log.warn("Acceso denegado (Security) en [{}]: {}", request.getRequestURI(), ex.getMessage());
                return ResponseEntity
                                .status(HttpStatus.FORBIDDEN)
                                .body(ResponseGlobal.error(HttpStatus.FORBIDDEN.value(),
                                                "No tienes permisos para realizar esta acción"));
        }

        // 403 - Acceso denegado personalizado de Dominio
        @ExceptionHandler(com.api.covoshcoffe.common.domain.exeption.AccessDeniedException.class)
        public ResponseEntity<ResponseGlobal<Void>> handleDomainAccessDenied(
                        com.api.covoshcoffe.common.domain.exeption.AccessDeniedException ex,
                        HttpServletRequest request) {
                log.warn("Acceso denegado (Domain) en [{}]: {}", request.getRequestURI(), ex.getMessage());
                return ResponseEntity
                                .status(HttpStatus.FORBIDDEN)
                                .body(ResponseGlobal.error(HttpStatus.FORBIDDEN.value(), ex.getMessage()));
        }

        // 400 - Excepción de dominio
        @ExceptionHandler(DomainException.class)
        public ResponseEntity<ResponseGlobal<Void>> handleDomainException(DomainException ex,
                        HttpServletRequest request) {
        ...
        public ResponseEntity<ResponseGlobal<Void>> handleGlobalException(Exception ex, HttpServletRequest request) {
                ...
                return ResponseEntity
                                ...
                                .body(ResponseGlobal.error(HttpStatus.INTERNAL_SERVER_ERROR.value(),
